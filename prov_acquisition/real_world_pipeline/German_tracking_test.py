...
def main(opt):
    ...
    tracker.df =tracker.df.replace({'checking': {'A11': 'check_low', 'A12': 'check_mid', 'A13': 'check_high',
                     ...
                     'label': {2: 0}})
    col = ['checking', 'credit_history', 'purpose', 'savings', 'employment', 'other_debtors', 'property', 'other_inst',
           'housing', 'job', 'phone', 'foreigner', 'label']

    #d = p.get_prov_feature_transformation(df, col)

    # OPERATION 1
    # More criptic values translating
    tracker.df['status'] = np.where(tracker.df.personal_status == 'A91', 'divorced',
                            np.where(tracker.df.personal_status == 'A92', 'divorced',
                                     np.where(tracker.df.personal_status == 'A93', 'single',
                                              np.where(tracker.df.personal_status == 'A95', 'single',
                                                       'married'))))
     # Translate gender values
    tracker.df['gender'] = np.where(tracker.df.personal_status == 'A92', 0,
                            np.where(tracker.df.personal_status == 'A95', 0,
                                     1))
    #tracker.df['prova']=np.zeros(len(tracker.df))
    #print(tracker.col_add)
    tracker.stop_space_prov(['personal_status'])
